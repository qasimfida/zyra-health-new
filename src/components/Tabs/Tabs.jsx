...
const Tabs = () => {
  ...
  const renderTabs = () => {
    return tabData.map((tab) => (
      <li key={tab.id} className="flex items-center w-full py-6 focus-within:z-10">
        <button
  ...
  const renderTabContent = () => {
    return tabData.map((tab) => (
      <div key={tab.id} id={`${tab.id}-tab`} className={`tab-content ${activeTab === tab.id ? 'block' : 'hidden'}`}>
        {/* Add your dummy data for each tab here */}
  ...
  return (
    <div className="bg-white rounded-2xl p-4">
      <div className="sm:hidden">
        {/* Add your select dropdown for mobile view here */}
      </div>
      <div>
